  ...
  useEffect(() => {
    const updateSplashImage = async () => {
      const serverImage = !Array.isArray(taxidoSettingData)
        ? taxidoSettingData?.taxido_values?.setting?.splash_screen_url
        : null;
      try {
  ...
  const proceedToNextScreen = useCallback(async () => {
    try {
      ...
      const requiredVersion = !Array.isArray(taxidoSettingData)
        ? taxidoSettingData?.taxido_values?.setting?.app_version
        : null;
      const forceUpdate = !Array.isArray(taxidoSettingData)
        ? taxidoSettingData?.taxido_values?.activation?.force_update == "1"
        : false;
